...
export const ListChat: React.FC= () => {
  ...
  const {conversations, setConversations, setSelectedChatId} = useConversationStore();
    ...
    <div className="px-0 relative w-full lg:w-1/4 sm:block mb-3 dark:bg-dark2 rounded-tl-lg">
      <div className="dark:bg-dark2 p-3 rounded-tl-md flex justify-between items-center">
        <div className="flex justify-between items-center gap-2">
          <Avatar
            src={user?.avatar ?? ""}
            cln={
              `w-[40px] h-[40px] object-cover border-none align-middle cursor-pointer ${colorAvatar}`
            }
          />
        ...
        <div className="bg-light0 cursor-pointer dark:bg-dark1 p-[6px] rounded-full text-dark1 dark:text-light1">
          <BiAddToQueue className="text-xl hover:text-mainColor" onClick={handleShowModalNewConversation}/>
        </div>
      </div>
      ...
      <div className="flex flex-col dark:bg-dark2 p-2 gap-2">
        ...
        <div className="h-[70vh] no-scrollbar min-w-full overflow-y-scroll">
          {conversations.length > 0 && conversations.map((e) => (
            <Link key={e._id}
      ...
      >
        <CreateNewConversation onCloseModal={handleCloseModalNewConversation} userId={user?._id ?? ""} onSuccess={(data: IConversation) => handleConversationUpdated(data)}/>
      </Modal>
    </div>
  );
};
