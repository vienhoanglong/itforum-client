...
export const ChatInformation: React.FC<ChatInformationProps> = ({userId, chatId, conversation}) => {
  ...
  return (
    ...
    >
       { conversation && conversation.map((e) =>e._id === chatId && (
        <div key={e._id} className="flex rounded-r-lg text-dark1 dark:text-light1 flex-col">
          <div className="mt-5 flex text-dark0 dark:text-light0">
            <h3 className="text-md flex-1 text-center">{e.nameConversation || `Group chat information`}</h3>
          </div>
              ...
              <div className="grid justify-items-center">
                ...
                <div className="flex flex-wrap max-w-[175px] gap-1 w-auto ">
                  {colorThemeChat.map((them, index)=>(
                    <div key={index} className={`w-10 h-10 rounded-lg  cursor-pointer ${
                      colorTheme === them.color
                        ? "border-4 border-blue-800"
                        : ""
                    } ${them.backgroundColor}`} onClick={()=>handleChangeTheme(them.color)}/>
                  ))}
                </div>
              </div>
          </div>
