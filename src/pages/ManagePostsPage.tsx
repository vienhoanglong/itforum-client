...
import PostUpdatePage from "@/modules/post/PostUpdate.tsx";

export const ManagePostsPage: React.FC = React.memo(() => {
  const { listAllTopic, getTopic } = useTopicStore();
  const { change, getListPost, getListPostsFromTrash, listAllPost } =
    usePostStore();
  ...
  const [postUpdate, setpostUpdate] = useState<string>("");
  ...
  useMemo(() => {
    setListPostDefault(
      listAllPost && listAllPost?.filter((post) => post.createdBy === user?._id)
    );
    setCurrentItems(
      listAllPost &&
        listAllPost
  ...
  const handlepostUpdate = (id: string) => {
    setpostUpdate(id);
  ...
  const handleHidden = async (id: string) => {
    handleConfirm(async () => {
      try {
        await changeStatusPost(id, HIDDEN);
        getListPost(0, 0, "desc");
  ...
  return (
    <LayoutSecondary>
      ...
      <div className=" h-auto mx-auto bg-light4 dark:bg-dark1 shadow-md p-4 rounded-3xl">
        <div className=" py-4">
          <h4 className="text-xl font-bold text-darker ">Post management</h4>
        </div>
        ...
        <Modal isOpen={isModalOpenAdd} onClose={handleCloseModal}>
          <PostAddNewPage onCancel={handleCloseModal} />
        </Modal>
        <Modal isOpen={isModalOpenUpdate} onClose={handleCloseModal}>
          <PostUpdatePage
            listAllTopic={listAllTopic}
            postId={postUpdate}
